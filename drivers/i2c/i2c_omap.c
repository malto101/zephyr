...
#define I2C_SA              0xac /* Target Address */
...
#define I2C_IE_AL   BIT(0)  /* Arbitration lost interrupt enable */
...
#define I2C_CON_STB       BIT(11) /* Start byte mode (controller) */
#define I2C_CON_MST       BIT(10) /* Controller/target mode */
#define I2C_CON_TRX       BIT(9)  /* TX/RX mode (controller only) */
#define I2C_CON_XA        BIT(8)  /* Expand address */
#define I2C_CON_STP       BIT(1)  /* Stop condition (controller only) */
#define I2C_CON_STT       BIT(0)  /* Start condition (controller) */

/* I2C_WE wakeup enable register */
#define I2C_WE_XDR_WE  BIT(14) /* TX drain wakeup */
#define I2C_WE_RDR_WE  BIT(13) /* RX drain wakeup */
#define I2C_WE_AAS_WE  BIT(9)  /* Address as target wakeup */
...
#define I2C_WE_ALL                                                                                 \
	(I2C_WE_XDR_WE | I2C_WE_RDR_WE | I2C_WE_AAS_WE | I2C_WE_BF_WE | I2C_WE_STC_WE |            \
	 I2C_WE_GC_WE | I2C_WE_DRDY_WE | I2C_WE_ARDY_WE | I2C_WE_NACK_WE | I2C_WE_AL_WE)
...
#define I2C_STAT_AAS  BIT(9)  /* Address as target */
...
static inline void i2c_omap_ack_stat(const struct device *dev, uint16_t stat)
{
	...
	i2c_omap_reg_set(cfg, I2C_STAT, stat);
}
...
static void i2c_omap_init_ll(const struct device *dev)
{
	...
	i2c_omap_reg_set(cfg, I2C_CON, 0);

	i2c_omap_reg_set(cfg, I2C_PSC, data->speed_config.pscstate);

	i2c_omap_reg_set(cfg, I2C_SCLL, data->speed_config.scllstate);
	i2c_omap_reg_set(cfg, I2C_SCLH, data->speed_config.sclhstate);

	i2c_omap_reg_set(cfg, I2C_WE, I2C_WE_ALL);

	i2c_omap_reg_set(cfg, I2C_CON, I2C_CON_EN);

	k_busy_wait(1);
	uint16_t iestate = (I2C_IE_XRDY | I2C_IE_RRDY | I2C_IE_ARDY | I2C_IE_NACK | I2C_IE_AL) |
			   ((data->current_msg.len) ? (I2C_IE_RDR | I2C_IE_XDR) : 0);
	i2c_omap_reg_set(cfg, I2C_IRQENABLE_SET, iestate);
}
...
static int i2c_omap_reset(const struct device *dev)
{
	...
	sysc = i2c_omap_reg_access(cfg, I2C_SYSC);

	i2c_omap_reg_set(cfg, I2C_CON, i2c_omap_reg_access(cfg, I2C_CON) & ~(I2C_CON_EN));

	timeout = k_uptime_get() + I2C_OMAP_TIMEOUT;

	i2c_omap_reg_set(cfg, I2C_CON, I2C_CON_EN);

	while (!(i2c_omap_reg_access(cfg, I2C_SYSS) & SYSS_RESETDONE_MASK)) {
	...
	i2c_omap_reg_set(cfg, I2C_SYSC, sysc);
...
static void i2c_omap_transmit_receive_data(const struct device *dev, uint8_t num_bytes)
{
	...
	while (num_bytes--) {
		if (data->receiver) {
			*data->current_msg.buf++ = i2c_omap_reg_access(cfg, I2C_DATA);
		} else {
			i2c_omap_reg_set(cfg, I2C_DATA, *(data->current_msg.buf++));
		}
	}
}
...
static void i2c_omap_resize_fifo(const struct device *dev, uint8_t size)
{
	...
	buf_cfg = i2c_omap_reg_access(cfg, I2C_BUF);
	if (data->receiver) {
		buf_cfg &= ~(0x3f << 8);
	...
	i2c_omap_reg_set(cfg, I2C_BUF, buf_cfg);
}
 ...
 */
static int __maybe_unused i2c_omap_get_scl(void *io_context)
{
...
static int i2c_omap_wait_for_bb(const struct device *dev)
{
	...
	while (i2c_omap_reg_access(cfg, I2C_STAT) & I2C_STAT_BB) {
 ...
 */
static int i2c_transfer_message_ll(const struct device *dev)
{
	const struct i2c_omap_cfg *cfg = dev->config;
	struct i2c_omap_data *data = dev->data;
	uint16_t bits, stat;
	int err = 0;
	do {
		bits = i2c_omap_reg_access(cfg, I2C_IE);
		stat = i2c_omap_reg_access(cfg, I2C_STAT);
		stat |= bits;
		...
		if (stat & I2C_STAT_NACK) {
			err |= I2C_STAT_NACK;
			i2c_omap_ack_stat(dev, I2C_STAT_NACK);
		}

		if (stat & I2C_STAT_AL) {
			err |= I2C_STAT_AL;
			i2c_omap_ack_stat(dev, I2C_STAT_AL);
		}

		if (stat & I2C_STAT_ARDY) {
			i2c_omap_ack_stat(dev, I2C_STAT_ARDY);
		}

		if (stat & (I2C_STAT_ARDY | I2C_STAT_NACK | I2C_STAT_AL)) {
			i2c_omap_ack_stat(dev, I2C_STAT_RRDY | I2C_STAT_RDR | I2C_STAT_XRDY |
						       I2C_STAT_XDR | I2C_STAT_ARDY);
			break;
		}
		/* Handle receive logic */
		if (stat & (I2C_STAT_RRDY | I2C_STAT_RDR)) {
			int buffer = i2c_omap_reg_access(cfg, (stat & I2C_STAT_RRDY) ? I2C_BUF
										     : I2C_BUFSTAT);
			i2c_omap_transmit_receive_data(dev, buffer);
			i2c_omap_ack_stat(dev, stat & I2C_STAT_RRDY ? I2C_STAT_RRDY : I2C_STAT_RDR);
			continue;
		}

		/* Handle transmit logic */
		if (stat & (I2C_STAT_XRDY | I2C_STAT_XDR)) {
			int buffer = i2c_omap_reg_access(cfg, (stat & I2C_STAT_XRDY) ? I2C_BUF
										     : I2C_BUFSTAT);
			i2c_omap_transmit_receive_data(dev, buffer);
			i2c_omap_ack_stat(dev,
					  (stat & I2C_STAT_XRDY) ? I2C_STAT_XRDY : I2C_STAT_XDR);
			continue;
		}
		...
		if (stat & I2C_STAT_ROVR) {
			...
			err |= I2C_STAT_ROVR;
			i2c_omap_ack_stat(dev, I2C_STAT_ROVR);
			break;
		}
		...
		if (stat & I2C_STAT_XUDF) {
			...
			err |= I2C_STAT_XUDF;
			i2c_omap_ack_stat(dev, I2C_STAT_XUDF);
			break;
		}
		if (!stat) {
			data->cmd_err |= -EAGAIN;
 ...
 */
static int i2c_transfer_message(const struct device *dev, struct i2c_msg *msg, bool polling,
				uint16_t addr)
{
	const struct i2c_omap_cfg *cfg = dev->config;
	struct i2c_omap_data *data = dev->data;
	...
	i2c_omap_reg_set(cfg, I2C_SA, addr);
	...
	i2c_omap_reg_set(cfg, I2C_CNT, msg->len);

	/* Clear FIFO buffers */
	control_reg = i2c_omap_reg_access(cfg, I2C_BUF);
	control_reg |= I2C_BUF_RXFIF_CLR | I2C_BUF_TXFIF_CLR;
	i2c_omap_reg_set(cfg, I2C_BUF, control_reg);
	...
	i2c_omap_reg_set(cfg, I2C_CON, control_reg);

	/* Wait for the transfer to complete, either by polling or using a semaphore */
	if (!polling) {
		result = k_sem_take(&data->cmd_complete, K_MSEC(I2C_OMAP_TIMEOUT));
		if (result == -EAGAIN) {
			/* Handle timeout case */
			time_left = -ETIMEDOUT;
		}
		...
		do {
			...
			for (uint8_t retries = 0; retries < 5; retries++) {
				if (i2c_omap_reg_access(cfg, I2C_STAT)) {
					break;
				}
				result = -EAGAIN;
			}
			/* Call a lower-level function to continue the transfer */
			result = i2c_transfer_message_ll(dev);
		} while (result == -EAGAIN);
		time_left = !result;
	}
	...
	if (data->cmd_err & (I2C_STAT_AL | I2C_STAT_NACK)) {
		...
		if (data->cmd_err & I2C_STAT_NACK) {
			...
			i2c_omap_reg_set(cfg, I2C_CON,
					 i2c_omap_reg_access(cfg, I2C_CON) | I2C_CON_STP);
 ...
 * message in the provided array and transfers them one by one using the i2c_transfer_message()
				  ...
				  bool polling, uint16_t addr)
{
	...
	for (int msg_idx = 0; msg_idx < num; msg_idx++) {
		ret = i2c_transfer_message(dev, &msg[msg_idx], polling, addr);
		if (ret < 0) {
			break;
		}
	}
...
{
	struct i2c_omap_data *data = dev->data;
	if (i2c_omap_reg_access(dev->config, I2C_STAT) & i2c_omap_reg_access(dev->config, I2C_IE) &
	    ~I2C_STAT_NACK) {
 ...
 * @return 0 on success, negative error code on failure.
 */
static int __maybe_unused i2c_omap_transfer_irq(const struct device *dev, struct i2c_msg msgs[],
