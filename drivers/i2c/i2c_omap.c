...
static inline void omap_i2c_write_reg(struct omap_i2c_dev *omap, int reg, uint16_t val)
{
	sys_write16(val, omap->base + (omap->regs[reg] << omap->reg_shift));
}

static inline uint16_t omap_i2c_read_reg(struct omap_i2c_dev *omap, int reg)
{
	return sys_read16(omap->base + (omap->regs[reg] << omap->reg_shift));
}
...
static int omap_i2c_reset(struct omap_i2c_dev *omap)
{
	...
	sysc = omap_i2c_read_reg(omap, I2C_SYSC);
	// Disable I2C controller before soft reset
	omap_i2c_write_reg(omap, I2C_CON, omap_i2c_read_reg(omap, I2C_CON) & ~(OMAP_I2C_CON_EN));

	timeout = k_uptime_get() + 1000;

	omap_i2c_write_reg(omap, I2C_CON, OMAP_I2C_CON_EN);

	while (!(omap_i2c_read_reg(omap, I2C_SYSS) & SYSS_RESETDONE_MASK)) {
	...
	omap_i2c_write_reg(omap, I2C_SYSC, sysc);
...
static int omap_i2c_init(const struct device *dev)
{
	...
	// prescaler Calculatation and clock periods for 100 kbps
...
static int omap_i2c_transmit_data(struct omap_i2c_dev *omap, uint8_t num_bytes, bool is_xdr)
{
	...
	while (num_bytes--) {
		...
		omap_i2c_write_reg(omap, I2C_DATA, w);
	}

	return 0;
}

static void omap_i2c_receive_data(struct omap_i2c_dev *omap, uint8_t num_bytes, bool is_rdr)
{
	uint16_t w;
	while (num_bytes--) {
		w = omap_i2c_read_reg(omap, I2C_DATA);
		*omap->buf++ = w >> 8;
		omap->buf_len--;
	}
}

static void omap_i2c_resize_fifo(struct omap_i2c_dev *omap, uint8_t size, bool is_rx)
{
	uint16_t buf;
	...
	omap->threshold = CLAMP(size, (uint8_t)1, omap->fifo_size);
	buf = omap_i2c_read_reg(omap, I2C_BUF);
	...
	omap_i2c_write_reg(omap, I2C_BUF, buf);

	/* calculate wakeup latency constraint for MPU */
	if (omap->set_mpu_wkup_lat != NULL) {
		omap->latency = (1000000 * omap->threshold) / (1000 * omap->speed / 8);
	}
}
...
{
	uint16_t stat;
	uint16_t mask = omap_i2c_read_reg(omap, I2C_IE);
	int count = 0;

	do {
		stat = omap_i2c_read_reg(omap, I2C_STAT);
		count++;
	} while (!(stat & mask) && count < 5);
}

static int omap_i2c_xfer_msg(struct omap_i2c_dev *omap, struct i2c_msg *msg, int stop, bool polling,
			     uint16_t addr)
{
	...
	omap->receiver = !!(msg->flags & I2C_MSG_READ);
	omap_i2c_resize_fifo(omap, msg->len, omap->receiver);
	omap_i2c_write_reg(omap, I2C_SA, addr);
	omap->buf = msg->buf;
	...
	omap_i2c_write_reg(omap, I2C_CNT, omap->buf_len);

	/* Clear the FIFO Buffers */
	w = omap_i2c_read_reg(omap, I2C_BUF);
	w |= OMAP_I2C_BUF_RXFIF_CLR | OMAP_I2C_BUF_TXFIF_CLR;
	omap_i2c_write_reg(omap, I2C_BUF, w);

	if (!polling) {
		k_sem_reset(&omap->cmd_complete);
	}
	...
	omap_i2c_write_reg(omap, I2C_CON, w);
	...
	if (omap->b_hw && stop) {
		...
		uint16_t con = omap_i2c_read_reg(omap, I2C_CON);
		while (con & OMAP_I2C_CON_STT) {
			con = omap_i2c_read_reg(omap, I2C_CON);
	...
	if (!polling) {
		...
		int result = k_sem_take(&omap->cmd_complete, OMAP_I2C_TIMEOUT);
	...
	if (omap->cmd_err & OMAP_I2C_STAT_NACK) {
		...
		w = omap_i2c_read_reg(omap, I2C_CON);
		w |= OMAP_I2C_CON_STP;
		omap_i2c_write_reg(omap, I2C_CON, w);
		return -EREMOTE;
	}
...
static inline void omap_i2c_ack_stat(struct omap_i2c_dev *omap, uint16_t stat)
{
	omap_i2c_write_reg(omap, I2C_STAT, stat);
}
...
static int omap_i2c_xfer_data(struct omap_i2c_dev *omap)
{
	...
	do {
		bits = omap_i2c_read_reg(omap, I2C_IE);
		stat = omap_i2c_read_reg(omap, I2C_STAT);
		stat &= bits;
		...
		if (stat & (OMAP_I2C_STAT_ARDY | OMAP_I2C_STAT_NACK | OMAP_I2C_STAT_AL)) {
			omap_i2c_ack_stat(omap, (OMAP_I2C_STAT_RRDY | OMAP_I2C_STAT_RDR |
...
static int omap_i2c_xfer_common(struct omap_i2c_dev *omap, struct i2c_msg msg[], int num,
				bool polling, uint16_t addr)
{
	...
	for (i = 0; i < num; i++) {
		r = omap_i2c_xfer_msg(omap, &msg[i], (i == (num - 1)), polling, addr);
		if (r != 0) {
			break;
		}
	}
...
static int omap_i2c_transfer(struct device *dev, struct i2c_msg *msgs[], uint8_t num_msgs,
			     uint16_t addr)

{
	int ret;
	struct omap_i2c_dev *omap = dev->config;
	ret = omap_i2c_xfer_common(omap, msgs[], num_msgs, false, addr);
	if (ret != 0) {
		LOG_WRN("unable to transfer");
	}
	return 0;
}
...
#define OMAP_I2C_INIT(inst)                                                                        \
	static int omap_i2c_##inst##_init(const struct device *dev);                               \
	DEVICE_DT_INST_DEFINE(inst, omap_i2c_##inst##_init, NULL, NULL, NULL, POST_KERNEL,         \
			      CONFIG_I2C_INIT_PRIORITY, omap_i2c_api);                             \
	static int omap_i2c_##inst##_init(const struct device *dev);
