...
static void omap_i2c_wait(const struct device *dev)
{
	...
	struct i2c_omap_data *data = dev->data;
	uint16_t stat;
	// Read the interrupt enable (IE) register to get the event mask
	uint16_t mask = omap_i2c_read_reg(cfg, data, I2C_IE);
	// Initialize a counter for the loop
	int count = 0;

	// Loop to wait for an I2C event or until the maximum count is reached
	do {
		LOG_DBG("Waiting for I2C event");
	stat = omap_i2c_read_reg(cfg, data, I2C_STAT);
	count++;
	// Continue looping while no event has occurred and the count is less than 5
...
static int __maybe_unused omap_i2c_transfer_irq(const struct device *dev, struct i2c_msg msgs[], uint8_t num_msgs,
			     uint16_t addr)
{
...
static const struct i2c_driver_api omap_i2c_api = {
	.transfer = omap_i2c_transfer_irq,
